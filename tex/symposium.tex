...
\begin{frame}{Agents \& Training Setup}
    \begin{itemize}
        ...
        \item TRPO is a policy gradient method that optimizes the policy directly while constraining updates using a trust region defined by the KL divergence.
...
\begin{frame}{Computing the FIM's Trace}
    \begin{itemize}
        ...
        \item Since the trace of a matrix is the sum of its diagonal entries $\sum_i F_{ii}$, we have \begin{equation*}
...
\begin{frame}{Algorithms: DQN and TRPO}
    \begin{itemize}
        ...
        \item It uses a surrogate objective function given by the expected advantage $\mathbb{E}_{s, a \sim \pi_{\theta_\text{old}}} \left[ \frac{\pi_\theta(a|s)}{\pi_{\theta_\text{old}}(a|s)} A^{\pi_{\theta_\text{old}}}(s, a) \right]$.
