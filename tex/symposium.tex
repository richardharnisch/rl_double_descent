...
\begin{frame}{Classical U-shaped Risk Curve}
    \begin{figure}
        \centering
        \includegraphics[height=0.5\textheight]{images/u-shaped.pdf}
...
\begin{frame}{Double Descent Risk Curve}
    % Double Descent risk curve, including U-shaped risk curve (the ``classical'' regime) together with the observed behavior from using high capacity function classes (the ``modern'' interpolating regime), separated by the interpolation threshold. Predictors to the right of the interpolation threshold have zero training risk.
    \begin{figure}
        \centering
        \includegraphics[height=0.4\textheight]{images/doubledescent.pdf}
...
\begin{frame}{Methods}
    ...
    \begin{figure}
        ...
        \end{minipage}
        \caption{Examples of randomly generated training and validation environments, with agent (blue) and goal (green) tiles.}
...
\begin{frame}{Limitations \& Future Work}
    \begin{itemize}
        ...
        \item One avenue for further work is exploring the transition between lacking generalization and perfect generalization.
        \item We observe no emerging generalization gap when training on 1,000 maps.
...
\begin{frame}{Environment Details}
    \begin{itemize}
        ...
        \item Every other tile has a 20\% chance of being blocked, with a guaranteed path.
        ...
        \item Episodes are truncated at 64 timesteps.
...
\begin{frame}{Training Configurations}
    \begin{itemize}
        \item On the following configurations, we trained agents of varying widths between 3 and 8 and depths between 4 and 1024:
...
\begin{frame}{The Fisher Information Matrix}
    \begin{itemize}
        \item The \textbf{Fisher Information Matrix} (FIM) is defined as:
              \begin{equation*}
                  F = \mathbb{E}_{a, s \sim \pi_\theta} \left[ \nabla_\theta \log \pi_\theta(a|s) \nabla_\theta \log \pi_\theta(a|s)^T \right]
              \end{equation*}
        \item $s, a$ are sampled from trajectories under the policy $\pi_\theta$, collected on the training maps.
...
\begin{frame}{Computing the FIM's Trace}
    \begin{itemize}
        \item We use Hutchinson's estimator for the trace to avoid forming the full FIM \parencite{hutchinson}:
              \begin{equation*}
                  \text{trace}(F) = \mathbb{E}[k^T F k]
              \end{equation*}
              where $k$ is a vector with entries sampled from a Rademacher distribution $\{-1, 1\}$.
        \item Fisher vector products are implemented as part of the TRPO algorithm, so we can compute $F k$ without forming $F$ explicitly.
        \item Our implementation samples 50 state-action pairs and 4 random vectors $k$ per pair to compute an empirical estimate of the trace, for 200 total samples.
    \end{itemize}
\end{frame}
