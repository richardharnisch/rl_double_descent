...
online stochastic MDP with a larger visited-state count and an LSTD-Q policy
evaluation/control loop. The current delayed-MDP implementation reaches the
interpolation region but does not recover through feature width 1024; the next
staged run should increase the state/context count and retain per-state value
error diagnostics alongside held-out return. Contexts, transitions, and
rewards must continue to be generated by interaction, with no saved
observation/label table. Until one of these staged experiments passes the fixed
return criterion, the scientifically correct conclusion remains “no genuine
